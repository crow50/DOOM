...
| 14.2.1 | 1 | Verify that all components are up to date, preferably using a dependency checker during build or compile time | Met | `pip-audit.yml` on every push and pull request, Trivy failing on HIGH/CRITICAL, Renovate, and a lockfile-drift diff in `diff-and-make-test.yml:13-15`; the runtime image applies Debian security updates at build time so a pinned base tag cannot hold OS packages back (D-39) | 1026 |
