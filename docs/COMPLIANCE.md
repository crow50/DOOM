...
| 1.2.2 | 2 | Partial | Verify that communications between application components, including APIs, middleware and data ... | `db` and `cache` require passwords (`config.py` `_database_url()` / `_redis_url()`) but the channel itself is unauthenticated - see 1.9.2 |
...
| 1.9.1 | 2 | **Not met** | Verify the application encrypts communications between components, particularly when these ... | All three internal hops are plaintext: `caddy->web` (`caddy/Caddyfile:34`), `web->db` and `web->cache` (`config.py` `_database_url()` / `_redis_url()`) |
...
| 9.2.2 | 2 | **Not met** | Verify that encrypted communications such as TLS is used for all inbound and outbound ... | `web->db` and `web->cache` are plaintext (`config.py` `_database_url()` / `_redis_url()`), as is `caddy->web`. The Postgres image is started without TLS enabled at all |
...
|---|:--:|---|---|---|:--:|
| 1.1.1 | 2 | Verify the use of a secure software development lifecycle that addresses security in all stages of development | Met | Threat model written before the code, 39 ADRs in `DECISIONS.md`, controls pinned by tests, 8 CI workflows | - |
...
| 1.2.2 | 2 | Verify that communications between application components, including APIs, middleware and data layers, are authenticated. Components should have the least necessary ... | Partial | `db` and `cache` require passwords (`config.py` `_database_url()` / `_redis_url()`) but the channel itself is unauthenticated - see 1.9.2 | 306 |
...
| 1.9.1 | 2 | Verify the application encrypts communications between components, particularly when these components are in different containers, systems, sites, or cloud providers | **Not met** | All three internal hops are plaintext: `caddy->web` (`caddy/Caddyfile:34`), `web->db` and `web->cache` (`config.py` `_database_url()` / `_redis_url()`) | 319 |
...
| 1.14.3 | 2 | Verify that the build pipeline warns of out-of-date or insecure components and takes appropriate actions | Met | `pip-audit.yml`, `trivy-image-scanning.yaml` (`exit-code: 1` on HIGH/CRITICAL), `semgrep.yml` (`--error`), `bandit-python-scanning.yml` (direct run, real exit code), Renovate, and a lockfile-drift diff - every one of them fails the build, which two of them could not until an audit checked | 1104 |
...
| 2.4.1 | 2 | Verify that passwords are stored in a form that is resistant to offline attacks | Met | Argon2id `t=3, m=64 MiB, p=4` (`security/passwords.py:34-40`, D-02) | 916 |
...
| 3.4.1 | 1 | Verify that cookie-based session tokens have the 'Secure' attribute set | Met | `SESSION_COOKIE_SECURE = True` (`config.py:173`) | 614 |
...
| 3.4.4 | 1 | Verify that cookie-based session tokens use the "__Host-" prefix so cookies are only sent to the host that initially set the cookie | Met | `SESSION_COOKIE_NAME = '__Host-doom_session'` (`config.py:171`) - Secure, `Path=/`, no `Domain` | 16 |
...
| ASVS | L | Requirement | Status | Evidence | CWE |
|---|:--:|---|---|---|:--:|
| 12.1.1 | 1 | Verify that the application will not accept large files that could fill up storage or cause a denial of service | Met | Flask 10 MB (`validation.py:231`, refused on `Content-Length` before the body is read) -> per-file check; Caddy's 12 MB `request_body` cap (`caddy/Caddyfile`) is the backstop for bodies with no declared length | 400 |
...
| 14.1.3 | 2 | Verify that server configuration is hardened as per the recommendations of the application server and frameworks in use | Met | `read_only`, `cap_drop: ALL`, `no-new-privileges`, non-root uid, tmpfs `/tmp` (`docker-compose.yml` (`web`, `cache` and `caddy` hardening blocks)) | 16 |
...
| 14.2.1 | 1 | Verify that all components are up to date, preferably using a dependency checker during build or compile time | Met | `pip-audit.yml` on every push and pull request that touches the lockfile, Trivy failing on HIGH/CRITICAL, Renovate, and a lockfile-drift diff in `diff-and-make-test.yml`; the runtime image applies Debian security updates at build time so a pinned base tag cannot hold OS packages back (D-39) | 1026 |
...
```bash
make test           # 305 tests pinning the controls above
